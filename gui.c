 ...
 */
void initialize_gui(struct Cell *ptr_field , struct Constants *info , char action) 
{
...
void make_txt_file(struct Cell *ptr_field , struct Constants *info) //ref 4
{
	...
	for (int j=0; j < info->no_horizontal_cels ; j++)
	{
		...
		fputc(visual_input , fPtr);
	 	char actual_input = *(ptr_field + find(j, i , info->no_horizontal_cels))->actual_value;
	 	if (actual_input == ' ')
	 	{
...
int main(int argc, char *argv[]) 
{
	...
	else if (argc == 3) //./main -f ../fase_2/saves/save_game.txt     this is how you open saved games
    {
		...
		{
			printf("\n these are the amount of chars %d" , total_chars);
			total_chars = total_chars / 2; //becouse we keep info from hidden and visual input
			vertical_cels--; //because the first line  is empty when using a save file generated by this code
			horizontal_cels = total_chars / vertical_cels;
			printf("\n these are the amount of vertical cels %d" , vertical_cels);
			printf("\n these are the amount of horizontal cels %d" , horizontal_cels);
			printf("\n these are the amount of bombs %d" , total_bombs);
			rewind(fPtr);
			struct Constants *field_info;
			field_info = (struct Constants *)malloc(sizeof(struct Constants));
			field_info->no_horizontal_cels = horizontal_cels ;
			field_info->no_vertical_cels = vertical_cels ;
			field_info->no_Bombs = total_bombs;
			field_info->bombs_set = 1;
			field_info->continue_game = 1;
			struct Cell *field;
			field = (struct Cell *)malloc(((vertical_cels + 25 ) * (horizontal_cels + 25 )) * sizeof(struct Cell)); //don't really know why segmentation faults occured here debug!!
			for (int i=0; i < field_info->no_vertical_cels ; i++)
			{	char eol; //dummy value
				fscanf(fPtr , "%c" , &eol);
				printf("\n");
				for (int j=0; j < field_info->no_horizontal_cels  ; j++)
				{
					int vis = getc(fPtr);
					int act = getc(fPtr);
					if (act == 'k')
					...
					*(field + find(j , i , field_info->no_horizontal_cels))->vissible_value = vis;
					*(field + find(j , i , field_info->no_horizontal_cels))->actual_value = act;
				}
			}
			printf("\n");
			fclose(fPtr);
			print_grid_frontend(field , field_info , '1');
			initialize_gui(field , field_info , '0');
   ...
   else if( argc == 7 ) 
   {
	...
	int opt, WIDTH_FIELD , HEIGHT_FIELD , AMOUNT_OF_BOMBS; //code mentioned in references. 2)
		...
		else
		{
			...
			initialize_gui(field , field_info , '1');
			while (should_continue) 
			{
				draw_window(field , field_info);
				read_input(field , field_info);
...
5) https://www.geeksforgeeks.org/c-program-to-count-the-number-of-characters-in-a-file/
 */
